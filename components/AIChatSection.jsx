...
const AIChatSection = () => {
  ...
  // FIXED: This is the critical useEffect that loads decision options when advisor is selected
  useEffect(() => {
  ...
  const handleLoadDecisionOptions = useCallback(async () => {
    ...
    try {
      console.log('handleLoadDecisionOptions called for:', currentAdvisor.id, 'step:', currentStep);
      const result = await loadDecisionOptions(currentAdvisor.id, currentStep, decisionPath);
      if (result && result.shouldGenerateRecommendation) {
