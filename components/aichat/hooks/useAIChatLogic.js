...
const sendEnhancedAIMessage = async (message, advisorId, userProfile, decisionPath, sentimentData = null) => {
  try {
    ...
    if (isAskingForHelp && currentDecisionStep < 0) {
      ...
      response.message = `Dziękuj�� za pytanie! Jako ${advisor.name}, mogę pomóc Ci z ${advisor.specialty.toLowerCase()} ` +
...
export const useAIChatLogic = () => {
  ...
  // Fallback options when the decision tree service is not available
  const getFallbackOptions = (advisorId, step) => {
    const advisor = ADVISORS.find(a => a.id === advisorId) || ADVISORS[0];
    const goalType = advisor.goal || "emergency_fund";
    
    console.log('Generating fallback options for:', { advisorId, goalType, step });
    ...
    if (goalType === "debt_reduction") {
      if (step === 0) {
        ...
        return [
          {
            id: "avalanche",
            text: "Metoda lawiny (najwyższe oprocentowanie)",
            value: "avalanche",
            question: "Jaką strategię spłaty preferujesz?"
          },
          {
            id: "snowball",
            text: "Metoda kuli śnieżnej (najmniejsze kwoty)",
            value: "snowball",
            question: "Jaką strategię spłaty preferujesz?"
          },
          {
    ...
    // Default fallback for any goal/step
    return [
      {
        id: "option1",
        text: "Opcja konserwatywna",
        value: "conservative",
        question: "Którą opcję wybierasz dla swojego celu finansowego?"
      },
      {
        id: "option2",
        text: "Opcja zrównoważona",
        value: "balanced",
        question: "Którą opcję wybierasz dla swojego celu finansowego?"
      },
      {
        id: "option3",
        text: "Opcja agresywna",
        value: "aggressive",
        question: "Którą opcję wybierasz dla swojego celu finansowego?"
      }
    ];
  ...
  const loadDecisionOptions = async (advisorId, currentStep, decisionPath) => {
    setLoading(true);
    try {
      console.log('loadDecisionOptions called:', { advisorId, currentStep, decisionPath });
      ...
      try {
        ...
        options = await decisionTreeService.processDecisionStep(
          advisorId, 
          currentStep, 
          formattedDecisionPath
        );
        console.log('Decision tree service returned options:', options);
      } catch (serviceError) {
        console.warn('Decision tree service failed, using fallback options:', serviceError);
        // Provide fallback options based on advisor and step
      ...
      // Provide basic fallback options
      const fallbackOptions = getFallbackOptions(advisorId, currentStep);
      setDecisionOptions(fallbackOptions);
      return { shouldGenerateRecommendation: false };
    } finally {
      setLoading(false);
    }
  ...
  const generateFinalRecommendation = async (advisorId, decisionPath, userProfile) => {
    ...
    try {
      ...
      try {
        ...
        recommendation = await decisionTreeService.generateReport(
          advisorId, 
          formattedDecisionPath, 
          userProfile
        );
      } catch (serviceError) {
        console.warn('Decision tree service failed for report generation, using fallback:', serviceError);
        // Generate a basic fallback recommendation
        const advisor = ADVISORS.find(a => a.id === advisorId) || ADVISORS[0];
        recommendation = {
          summary: `Na podstawie Twoich wyborów przygotowaliśmy plan dla celu: ${getGoalName(advisor.goal)}. Ten plan został dostosowany do Twojej sytuacji finansowej.`,
          steps: [
            "Przeanalizuj swoją obecną sytuację finansową",
            "Ustaw realistyczne cele czasowe",
            "Rozpocznij systematyczne oszczędzanie",
